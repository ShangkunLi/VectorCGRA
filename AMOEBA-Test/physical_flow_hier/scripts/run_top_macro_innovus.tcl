# Place and route the 4x4 multi-CGRA top using one hardened core macro.
...
if {![file exists $TOP_MACRO_NETLIST]} {
    puts stderr "Top macro netlist not found: $TOP_MACRO_NETLIST. Run scripts/run_innovus_macro.sh."
    exit 1
}
if {![file exists $SDC_FILE]} {
    puts stderr "Top SDC not found: $SDC_FILE. Run scripts/run_dc_hier.sh first."
    exit 1
}
if {![file exists $CORE_LEF]} {
    puts stderr "Core LEF not found: $CORE_LEF. Run scripts/run_core_innovus.tcl first."
    exit 1
}

set lefs [concat $lefs [list $CORE_LEF]]
source scripts/mmmc_setup.tcl

set rptDir "summaryReport/top_macro"
set encDir "enc/top_macro"
set defDir "def/top_macro"
...
set init_verilog $TOP_MACRO_NETLIST
...
amoeba_apply_core_macro_grid $CORE_MODULE

setDesignMode -topRoutingLayer $TOP_ROUTING_LAYER
setDesignMode -bottomRoutingLayer 2

echo "Physical Design Stage, Core Area (um^2), Standard Cell Area (um^2), Macro Area (um^2), Total Power, Wirelength(um)" > ${DESIGN}_MACRO_DETAILS.rpt
set rpt_post_synth [extract_report postSynth]
echo "$rpt_post_synth" >> ${DESIGN}_MACRO_DETAILS.rpt
...
place_opt_design -out_dir $rptDir -prefix top_macro_place
optDesign -preCTS
...
echo "$rpt_post_route" >> ${DESIGN}_MACRO_DETAILS.rpt

optDesign -postRoute

set rpt_post_route_opt [extract_report postRouteOpt]
echo "$rpt_post_route_opt" >> ${DESIGN}_MACRO_DETAILS.rpt

verify_connectivity -error 0 -geom_connect -no_antenna
verify_drc -limit 0

summaryReport -noHtml -outfile ${rptDir}/post_route.sum
write_area_breakdown_csv reports/innovus_top_macro_area_breakdown.csv
