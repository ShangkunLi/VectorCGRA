...
```text
Baseline RTL -> DC -> gate netlist -> Innovus -> placed/routed layout + PPA
```
...
```text
../generated/BaselineMultiCgra4x4Cgra2x2RTL.v
```

The default top module is:

```text
BaselineMultiCgra4x4Cgra2x2RTL
```
...
This requires the mapped `results/baseline.ddc` produced by `run_dc.sh`. It
sets all non-clock primary inputs and all sequential output pins to 0.2
transitions per `sys_clk` cycle with static probability 0.5. This matches the
Innovus report fields `Primary Input Activity: 0.200000` and
...
```text
syn_handoff/baseline.v          synthesized netlist from DC
syn_handoff/baseline.sdc        SDC emitted by DC
...
def/baseline.def                routed DEF
enc/baseline.enc                final Innovus database
```
